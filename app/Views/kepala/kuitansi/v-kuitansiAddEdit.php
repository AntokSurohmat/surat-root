                ...
                <div class="card card-outline card-primary">
                    ...
                    <form class="form-horizontal" role="form" id="form-addedit" autocomplete="off" onsubmit="return false" enctype="multipart/form-data">
                        ...
                        <div class="card-footer" style="text-align:center;">
                            <a type="button" href="<?= base_url('') ?>/Kepala/Kuitansi" class="btn btn-secondary mr-2"><i class="fas fa-arrow-left"></i>&ensp;Back</a>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var url_destination = '<?= base_url('Kepala/Kuitansi/getNoSpd') ?>';
        ...
        $("#namaPegawaiKuitansiForm").change(function() {
            ...
            var url_destination = '<?= base_url('Kepala/Kuitansi/getDetailPegawaiNoSpd') ?>';
            $.ajax({
        ...
        $("#pejabatKuitansiForm").select2({
            ...
            ajax: {url: url_destination,type: "POST",dataType: "JSON",delay: 250,
                data: function(params) {
                    return {searchTerm: params.term,bendahara: kepalaNip,csrf_token_name: $('input[name=csrf_token_name]').val()};
                },
                processResults: function(response) {
                    $('input[name=csrf_token_name]').val(response.csrf_token_name);
                    return {results: response.data,};
                    ...
                    if (data.success==true) {
                        ...
                        swalWithBootstrapButtons.fire({
                            ...
                            if (result.isConfirmed) {window.location.href = data.redirect;
                                ...
                                }else{window.location.replace("<?= base_url('Kepala/Kuitansi/new')?>");}
        ...
        function update() {
            if ($('#methodPage').val() === "Update" && $('#hiddenIDPage').val() != "") {
                var id = $('#hiddenIDPage').val();var url_destination = "<?= base_url('Kepala/Kuitansi/single_data') ?>";
                $.ajax({
