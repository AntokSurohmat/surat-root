...
<body>
    ...
    <table border="0" width= 100% cellpadding=2 cellspacing=0 style="margin-top: 5px; text-align:center">
        <tr>
            ...
            <td width=50%>
                <p style="font-size:20px;text-align:center;line-height: 1em;font-weight:800" class="mb-0 mr-5">PEMERINTAH KABUPATEN CIREBON</p>
    ...
    <table  border="0" class="table minimpadding" style="font-size: 14px;width:80%">
        <tbody>
            ...
            <tr>
                ...
                <td style="width: 1%;">:</td>
                <td ><p>Biaya Perjalanan Dinas <span><?= $wilayah['jenis_wilayah']?></span> dalam rangka <span><?= $untuk ?></span> di <span><?= $instansi['nama_instansi']?></span> selama <span><?= $lama ?></span> hari pada tanggal <span><?= date('d-m-Y', strtotime($awal)) ?></span> sampai <span><?= date('d-m-Y', strtotime($akhir))?></span> a/n <span><?= $pegawai['nama']?></span> </p></td>
            </tr>
        </tbody>
    </table>
    ...
    <!-- <script>
        window.ready(function() {
            $('#bkuKuitansiModalView').append(': OK');
        });
    </script> -->
</body>
