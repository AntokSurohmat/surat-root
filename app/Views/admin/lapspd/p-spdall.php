...
<head>
	...
	<title>Print All Data SPD</title>
	...
	<?php foreach ($spd as $spd_data) : ?>
		...
		<table border="0" width=100% style="padding-left:70%;margin-top:10px;margin-bottom:10px;">
			<tbody>
				...
				<tr>
					...
					<td width=5% style="padding:5px"> : </td>
					<td style="padding:5px 10px;"><span><?= $spd_data->kode ?></span></td>
				</tr>
			</tbody>
		</table>
			...
			<tbody>
				<tr>
					...
					<td colspan="2" style="padding: 5px 10px;">:
						<?php
						$i = $n - 1;
						foreach ($diperintah as $key =>  $diperintah_data) : ?>
							<?php foreach ($diperintah_data as $diperintah_isi) : ?>
								<?php if ($key == $i) : ?>
									<?= $diperintah_isi->nama ?>
								<?php endif; ?>
							<?php endforeach; ?>
				...
				<tr>
					...
					<td colspan="2" style="padding: 5px 10px;">:
						<?php
						$i = $n - 1;
						foreach ($pegawai as $key =>  $pegawai_data) : ?>
							<?php foreach ($pegawai_data as $pegawai_isi) : ?>
								<?php if ($key == $i) : ?>
									<?= $pegawai_isi->nama ?>
								<?php endif; ?>
							<?php endforeach; ?>
						...
						<ul style="margin: 0px;padding-top:5px;padding-bottom:5px;padding-left:10px;">
							...
							<li style="padding-top: 5px;">:
								...
								foreach ($pegawai as $key =>  $pegawai_data) : ?>
									<?php foreach ($pegawai_data as $pegawai_isi) : ?>
										<?php if ($key == $i) : ?>
											<?= $pegawai_isi->nama_jabatan ?>
										<?php endif; ?>
									<?php endforeach; ?>
								<?php endforeach ?>
								/ Disperdagin Kabupaten Cirebon
							</li>
							<li style="padding-top: 5px;">: <?= $spd_data->tingkat_biaya ?></li>
						</ul>
					</td>
				</tr>
				<tr>
					...
					<td colspan="2" style="padding: 5px 10px;">: <?= $spd_data->untuk ?></td>
				</tr>
				<tr>
					<th style="width:5%;">5.</th>
					<td style="width: 40%;padding: 5px 10px;">Alat angkutan yang dipergunakan</td>
					<td colspan="2" style="padding: 5px 10px;">: <?= $spd_data->jenis_kendaraan ?></td>
				</tr>
				<tr>
					...
					<td colspan="2">
						<ul style="margin: 0px;padding-top:0px;padding-bottom:0px;padding-left:10px;">
							...
							<li style="padding-top: 5px;">:
								<?php
								$i = $n - 1;
								foreach ($instansi as $key =>  $instansi_data) : ?>
									<?php if ($key == $i) : ?>
				...
				<tr>
					...
					<td colspan="2">
						<ul style="margin: 0px;padding-top:2px;padding-bottom:2px;padding-left:10px;">
							<li style="padding-top: 5px;">: <?= $spd_data->lama ?> Hari</li>
							<li style="padding-top: 5px;">: <?= date('d-m-Y', strtotime($spd_data->awal)) ?></li>
							<li style="padding-top: 5px;">: <?= date('d-m-Y', strtotime($spd_data->akhir)) ?></li>
						</ul>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td style="width: 5%;"></td>
					<td style="width:20%;padding-top:5px;padding-left:10px;"></td>
					<td style="width:25%;padding-top:5px;padding-left:10px;"></td>
					<td style="width:25%;padding-top:5px;padding-left:10px;"></td>
				</tr>
						...
						<ul ul style="margin: 0px;padding-top:2px;padding-bottom:2px;padding-left:10px;">
							...
							<li>
								<ul style="margin: 0px;padding-top:2px;padding-bottom:2px;padding-left:0px;">
									...
									<li style="padding-top: 5px;">: <?= $spd_data->kode_rekening ?></li>
								</ul>
							</li>
						</ul>
					</td>
				...
				<tr>
					...
					<td colspan="2" style="padding:5px 10px;">: <?= $spd_data->keterangan ?></td>
				</tr>
			</tbody>
		</table>
		...
		<table border="0" width="100%" class="table" style="padding-left:50%;margin-top:10px;">
			...
			<tr>
				...
				<td style="padding:5px;width:2%"> : </td>
				<td style="padding:5px;"><?= date_indo(date('Y-m-d', strtotime($spd_data->created_at))) ?></td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="3" style="padding:4px;text-align:center">
					...
					foreach ($diperintah as $key =>  $diperintah_data) : ?>
						<?php foreach ($diperintah_data as $diperintah_isi) : ?>
							<?php if ($key == $i) : ?>
								<span style="font-weight: 800;font-size:16px"><?= $diperintah_isi->nama ?></span><br>(<?= $diperintah_isi->nip ?>)
				</td>
			<?php endif; ?>
		<?php endforeach; ?>
	<?php endforeach ?>
				...
				<td style="width: 50%; padding:2px;">
					<table width=100% border="0">
						<tr>
							...
							<td style="padding:4px 10px;"><?= $spd_data->kode ?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td style="padding:4px 10px;width: 2%;">:</td>
							<td style="padding:4px 10px;"><?= date('d-m-Y', strtotime($spd_data->awal)) ?></td>
						</tr>
						<tr>
							<th width=10% style="padding:4px 10px;"></th>
						...
						<tr>
							<td colspan="4" style="padding:4px 10px;text-align:center;">
								<?php
								$i = $n - 1;
								foreach ($diperintah as $key =>  $diperintah_data) : ?>
									<?php foreach ($diperintah_data as $diperintah_isi) : ?>
										<?php if ($key == $i) : ?>
											<span style="font-weight: 800;"><?= $diperintah_isi->nama ?></span><br>(<?= $diperintah_isi->nip ?>)
							</td>
						<?php endif; ?>
					<?php endforeach; ?>
				<?php endforeach ?>
		...
		<tr>
			<td style="width: 50%; padding:2px;">
				<table width=100% border="0">
					<?php
					$i = $n - 1;
					foreach ($json as $keys => $json_data) : ?>
						<?php foreach ($json_data as $row => $content) : ?>
							<?php foreach ($content as $urutan => $isi) : ?>
								<?php if ($keys == $i && $row == 0) : ?>
									<?php if ($urutan == 'tibadi') : ?>
										<tr>
											<th width=10% style="padding:4px;">II.</th>
											<td style="padding:4px;width:40%;">Tiba di</td>
											<td style="padding:4px;width: 2%;">:</td>
											<td style="padding:4px;width:40%;"><?= $isi ?></td>
										</tr>
									<?php endif; ?>
									<?php if ($urutan == 'tanggaltiba') : ?>
										<tr>
						...
						<?php foreach ($json_data as $row => $content) : ?>
							<?php foreach ($content as $urutan => $isi) : ?>
								<?php if ($keys == $i && $row == 0) : ?>
									...
									<?php endif; ?>
									<?php if ($urutan == 'tanggalberangkat') : ?>
										<tr>
											<th width=10% style="padding:4px;"></th>
			...
			<td style="width: 50%; padding:2px;">
				<table width=100% border="0">
					...
					<tr>
						...
						<td style="padding:4px 10px;width:40%;">Pada Tanggal</td>
						<td style="padding:4px 10px;width: 2%;">:</td>
						<td style="padding:4px 10px;"><?= date('d-m-Y', strtotime($spd_data->akhir)) ?></td>
					</tr>
					<tr>
						<th width=10% style="padding:4px 10px;"></th>
		...
		<tr>
			<td colspan="2" class="text-justify">
				<p><b>VII.</b> Pejabat yang berwenang menerbitkan SPD, pegawai yang melakukan perjalanan dinas, para pejabat yang mengesahkan tanggal berangkat/tiba serta Bendaharawan yang bertanggung jawab, berdasarkan pengaturan-pengaturan Keuangan Negara apabila Negara mendapatkan akibat kesalahan, kelupaannya.</p>
			</td>
		</tr>
		</table>
